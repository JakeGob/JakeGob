# Hey, I'm Jake 👋

CS student at Dakota State University · Software Design Student @ Daktronics · Expected grad: August 2026

## About

I'm a software engineering student who likes building things that actually work. Full-stack web apps, real-time systems, whatever the problem calls for. Currently working at Daktronics doing Salesforce feature development and C# code. I also run a home lab for fun and to keep my networking knowledge from going stale.

I'm comfortable across the stack and enjoy the full cycle: building, testing, and figuring out what broke.

## Tech

![C](https://img.shields.io/badge/C-00599C?style=flat&logo=c&logoColor=white)
![C++](https://img.shields.io/badge/C++-00599C?style=flat&logo=cplusplus&logoColor=white)
![C#](https://img.shields.io/badge/C%23-239120?style=flat&logo=csharp&logoColor=white)
![JavaScript](https://img.shields.io/badge/JavaScript-F7DF1E?style=flat&logo=javascript&logoColor=black)
![React](https://img.shields.io/badge/React-20232A?style=flat&logo=react&logoColor=61DAFB)
![Node.js](https://img.shields.io/badge/Node.js-339933?style=flat&logo=nodedotjs&logoColor=white)
![SQL](https://img.shields.io/badge/SQL-4479A1?style=flat&logo=postgresql&logoColor=white)
![Salesforce](https://img.shields.io/badge/Salesforce-00A1E0?style=flat&logo=salesforce&logoColor=white)
![Git](https://img.shields.io/badge/Git-F05032?style=flat&logo=git&logoColor=white)
![Linux](https://img.shields.io/badge/Linux-FCC624?style=flat&logo=linux&logoColor=black)

## Projects

### 🗺️ Memory Map
Full-stack React web app for tracking shared memories. Interactive map, photo albums, and a bucket list.  
**Stack:** React · Vite · Tailwind CSS · Supabase · Leaflet

### 🚗 Car Maintenance Tracker
Vehicle maintenance logger with a REST API backend and persistent local storage.  
**Stack:** Express · Better-SQLite3 · HTML/CSS/JS

### 📷 Cafeteria Occupancy Monitor *(team project)*
Real-time people detection system integrating a YOLO model with a live web dashboard.  
**Stack:** Python · Ultralytics YOLO · HTML/CSS/JS · SQLite

### 🖥️ Home Lab
Self-hosted network setup running Pi-hole, Nginx Proxy Manager, a Cloudflare tunnel, and Twingate on a Raspberry Pi 5.  
**Stack:** Linux · Docker · Raspberry Pi

## Reach me

📧 jmgoldfish@outlook.com · [LinkedIn](https://www.linkedin.com/in/jake-goblirsch-a8b597311)
